...
def add_friend(user, friend):
    cursor.execute("SELECT * FROM friends WHERE user_mobile=? AND friend_mobile=?", (user, friend))
    if cursor.fetchone():
        return "Already Added"

    cursor.execute(
        "INSERT INTO friends (user_mobile, friend_mobile, added_on) VALUES (?, ?, ?)",
...
if st.session_state.user:
    ...
    if option == "Dashboard":
        ...
        cursor.execute("SELECT friend_mobile FROM friends WHERE user_mobile=?", (st.session_state.user,))
